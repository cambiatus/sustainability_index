...
-}
type alias Vector =
    { x : Float, y : Float, label : String, info : String }
...
add v w =
    Vector (v.x + w.x) (v.y + w.y) (v.label ++ w.label) (v.info ++ w.info)
...
mul c v =
    Vector (c * v.x) (c * v.y) v.label v.info
